...
ctrl_C = np.array([
    [30, 35, 42],     # grout / deep crack  (밝게 — 검은 그물망 방지)
    [40, 47, 56],     # shadow
    [50, 58, 68],     # mid
    [62, 71, 82],     # light
    [76, 87, 99],     # highlight (낮춤 — 바닥 대비 압축)
], float)
...
for k in range(N):
    ...
    inset = 0.028 + wob                        # 더 얇은 줄눈 (그물망 완화)
    # occasional corner chip: extra bite out of one corner
    if rng.random() < 0.25:
        ca = rng.uniform(-3.14, 3.14)
        chip = np.exp(-((((ang - ca + 3.14) % 6.28) - 3.14) ** 2) / 0.12) * rng.uniform(0.03, 0.07)
        inset = inset + chip
    stone = d > inset
    ...
    L -= rim * 0.22                             # 베벨 약화 (줄눈 굵어 보임 방지)

    # per-tile character
    dark = rng.random() < 0.12                  # damp / dark flagstone
    L += (-0.10 if dark else rng.uniform(-0.07, 0.07))
...
sheet.save('tileset_iso_stone.png')
    ...
    if kind == 0:   # crack network (dark)
        for _ in range(rng.integers(2, 4)):
            ...
            for _ in range(rng.integers(20, 40)):
                ...
                rgba[iy, ix, :3] = [8, 9, 12]; rgba[iy, ix, 3] = 110
        ...
        m = m * (0.5 + 0.5 * nz)
        rgba[..., 0] = 48; rgba[..., 1] = 78; rgba[..., 2] = 46; rgba[..., 3] = m * 90
        ...
        rgba[..., 0] = 14; rgba[..., 1] = 20; rgba[..., 2] = 30; rgba[..., 3] = m * 90
    else:            # rubble / pebbles
...
prev.convert('RGB').save('_iso_preview.png')
