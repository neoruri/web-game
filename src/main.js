...
const BOW_OFF = {
  ...
  // run(걷기)은 여섯 열이 **전부 같은 값**이다. 걷기는 상체가 거의 안 흔들려서
  // — 실측 몸통 가로 진폭 2.3px(화면 1.3px) — 등에 멘 활이 프레임마다 움직일
  // 이유가 없다. 프레임마다 각도를 바꿔 까딱이게 해봤더니 오히려 활이 등에서
  // 흔들려 보여 45° 고정으로 되돌렸다. 배열 구조는 유지한다 — back_run·attack 이
  // 들어오면 열마다 달라질 수 있다.
  run: [
...
class GameScene extends Phaser.Scene {
  ...
  setupPlayerSprite() {
    ...
    //   run 행은 내용이 **걷기**다(달리기는 보폭이 커서 발이 바닥에 안 붙어 보였다).
    //   애니 키는 run 그대로 둔다 — 호출부가 여럿이라 이름만 바꾸면 손댈 곳이 는다.
    //   6프레임 중 4개만 쓴다. 열별 보폭%: 0/32/28/15/22/29 → 열2·4는 열5·3과 겹쳐
    //   다 넣으면 같은 자세가 두 번 지나가 걸음이 뭉갠다.
    //   [0,1,3,5] = 0→32→15→29 (모음→크게→중간→크게) 로 걸음마다 리듬이 선다.
    //   열2·4는 6프레임으로 늘릴 여지로 시트에 남겨둔다.
    ...
    //
    // run fps 12 — 발 미끄러짐 기준. 화면 최대 보폭 17.9px(셀 32.6 × 0.55),
    //    이동 100px/s, 4프레임 한 사이클에 걸음 2회:
    //      fps11 → 걸음당 18.2px(보폭 대비 1.01, 이론상 최적)
    //      fps12 → 16.7px(0.93)  ← 채택. 11과 눈으로 구분이 안 되고 다른 애니와 값이 같다
    //      fps14 → 14.3px(0.80, 발이 20% 끌린다)
